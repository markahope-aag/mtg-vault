...
The top navigation has five sections: **Dashboard**, **Inventory**, **Decks**,
**Import**, and **System**. The app is dark by default; the sun/moon button in
the top-right toggles light mode.

Press **⌘K** (Ctrl+K) anywhere to open the card search palette.
...
location, acquired price, etc. The card appears in the list immediately.
...
The list loads 200 cards at a time — **Load more** pulls the rest. A
...
- The art shows a printing **you own** by default.
...
- **Coach** — a heuristic build check. Every slot (Ramp, Removal, Card draw,
  ...
  what's **in the deck** (with Remove) and **owned candidates** to add. Basic
  lands have a quantity input and show how many you own.
- **Strategy** — an AI analysis (Claude). Click **Analyze deck** for
  ...
  **Worth acquiring** (a shopping list regardless of ownership). Cached;
  **Re-analyze** when the deck changes.
- **Acquire** — a cost-to-build rollup: every non-basic card you don't own
  enough copies of, with a price total.
...
2. **Configure** — set a default location and import mode.
